    ...
    <v-card class="pa-3">
      <v-card-text class="pb-0">
        ...
        <Field v-slot="{ field, errors }" name="email">
          <v-text-field
            v-bind="field" :label="t('auth.email')" type="email" required :error-messages="errors"
            append-inner-icon="mdi-account" autocomplete="username email" data-test="login.email" />
        </Field>
        <Field v-slot="{ field, errors }" name="password">
          <v-text-field
            v-bind="field" append-inner-icon="mdi-lock" :label="t('auth.password')" type="password"
            :error-messages="errors" autocomplete="current-password" data-test="login.password" />
        </Field>
      </v-card-text>
          ...
          <v-row no-gutters>
            <v-col cols="12">
              <v-btn
                ...
                :disabled="Object.keys(formErrors).length !== 0" type="submit" data-test="login.submit">
                {{ t('auth.login') }}
              </v-btn>
            </v-col>
            <v-col cols="12">
  ...
  mutate({ email, password })
    .then((response) => {
      ...
      onLoginApollo(
        ...
        response?.data.logIn.expires_in
      )
