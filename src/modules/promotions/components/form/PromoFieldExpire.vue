<template>
  <div data-test="promo.expire_at.block">
    ...
    <input id="expire_at" v-model="value" type="date" :min="dayjs().format('YYYY-MM-DD')" data-test="promo.expire_at" />
    <v-error-message name="expire_at" />
  </div>
</template>
...
const { t } = useI18n()
const { value } = useField('expire_at')
</script>
