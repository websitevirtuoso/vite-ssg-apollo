      ...
      <v-row>
        ...
        <v-col cols="12" md="3">
          <v-select
            ...
            density="comfortable"
            clearable
            data-test="filter.user.status"
          />
        </v-col>
        <v-col cols="12" md="3">
          <filter-by-text
            v-model="rawFilter.first_name"
            :label="t('messages.first_name')"
            density="compact"
            data-test="filter.user.first_name"
          />
        </v-col>
        <v-col cols="12" md="3">
          <filter-by-text
            v-model="rawFilter.last_name"
            :label="t('messages.last_name')"
            density="compact"
            data-test="filter.user.last_name"
          />
        </v-col>
        <v-col cols="12" md="3">
        ...
        <v-col cols="12" md="3">
          <v-select
            v-model="rawFilter.notify"
            :label="t('messages.notify')"
            clearable
            data-test="filter.user.notify"
            :items="[
              { value: true, title: t('action.yes') },
              { value: false, title: t('action.no') },
            ]"
          />
        </v-col>
        <v-col cols="12" md="3">
        ...
        <v-col cols="12" md="3">
          <roles-query v-slot="{ items, loading }">
            <v-select
              v-model="rawFilter.role_names"
              :items="items"
              multiple
              item-title="display_name"
              item-value="name"
              :label="t('messages.role', 2)"
              :loading="loading"
              clearable
              data-test="filter.user.roles"
            />
          </roles-query>
        </v-col>
        <v-col cols="12" md="4">
