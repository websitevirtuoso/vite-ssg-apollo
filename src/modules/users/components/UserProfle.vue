  ...
  <v-card tile style="margin-top: 15%">
    <v-card-actions>
      <v-btn v-if="can('update', 'user')" icon>
        <v-icon color="primary" @click="router.push({ name: 'user-update', params: { id: user.id }})">
          mdi-pencil
        </v-icon>
      </v-btn>
      <v-spacer />
    ...
    <v-card-text class="text-h5 text-center mt-3 py-0">
    ...
    <v-list dense>
      ...
      <v-list-item>
        <template #default>
          <v-list-item>
            <v-list-item-title>
              {{ user.city.state.country.name }}, {{ user.city.state.name }}, {{ user.city.name }}
            </v-list-item-title>
