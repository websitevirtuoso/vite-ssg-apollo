...
describe('Create', () => {
  ...
  it('should display required fields', () => {
    ...
    cy.getBySel('tab-address').click()
    cy.getBySel('user.country').errorValidation('Country is a required field')
    ...
    cy.getBySel('user.status').errorValidation('Status is a required field')
  ...
  it.only('should create', () => {
    ...
    users.generateUser().then((user: CyUser) => {
      ...
      user.phone.forEach((phone, index) => {
        cy.getBySel(`user.phone[${index}]`).type(String(user.phone))
